...
import { queryAllByRole, render, RenderResult, waitFor } from '@testing-library/react';
...
describe('ConversationAreasList', () => {
    ...
    if (areas.length === 0) {
      ...
      for (let i = 0; i < areas.length; i += 1) {
        ...
        if (players) {
          ...
          if (parentElement) {
            const playerNodes = queryAllByRole(parentElement, 'listitem');
      ...
      for (let playerNum = 0; playerNum < NUM_PLAYERS_PER_AREA; playerNum += 1) {
        playersInThisArea.push(
          new Player(
            ...
            [],
            [],
            `area${areaID}.${playerNum}playerEmail:${nanoid()}`,
          ),
        );
      }
  ...
  describe('Without checking the player IDs', () => {
  ...
  it('When checking usernames, it sorts conversation areas by label and lists player usernames in provided order', async () => {
    const areasProvidedInSortOrder = areas;
    useConversationAreasSpy.mockReturnValue(areasProvidedInSortOrder);
    const renderData = renderConversationAreaList();
  ...
  describe('Registration of listener with useEffect', () => {
  ...
  describe('Updating occupants from the onOccupantsChange listener', () => {
    beforeEach(() => {
      useConversationAreasSpy.mockReturnValue(areas);
    });
